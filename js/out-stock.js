 window.onload = function () {
      ...
      function renderProducts(filteredProductsWithIndex) {
        ...
        filteredProductsWithIndex.forEach(({ product, idx }) => {
          ...
          if (isOutOfStock) {
            productElement.style.color = "gray";
            productElement.style.backgroundColor = "#f8d7da";
          }
          ...
          productElement.innerHTML = `
            ...
            <div>
              ...
              <button id="plus-${idx}" onclick="quantityPlus(${idx})" ${isOutOfStock ? "disabled" : ""}>+</button>
            </div>
            <p class="price">
              ${
                product.discount &&
            ...
            <button onclick="addToCart(${idx})" ${isOutOfStock ? "disabled" : ""}>${isOutOfStock ? "Out of Stock" : "Add to Cart"}</button>
          `;
